...
export async function DELETE(request, { params }) {
    try {
        ...
        if (!id) return NextResponse.json({ error: 'Interview result ID is required' }, { status: 400 });

        const db = await getDb();
        await db.delete(interviewResultsTable).where(eq(interviewResultsTable.id, parseInt(id)));
